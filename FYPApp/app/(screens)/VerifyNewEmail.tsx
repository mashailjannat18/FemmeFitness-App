...
const VerifyNewEmail: React.FC = () => {
  const [code, setCode] = useState<string>('');
  const [loading, setLoading] = useState<boolean>(false);
  const [resendLoading, setResendLoading] = useState<boolean>(false);
  const [error, setError] = useState<string>('');
  const router = useRouter();
  const { newEmail, currentEmail } = useLocalSearchParams<{ newEmail: string; currentEmail: string }>();
  const { verifyNewEmailCode, resendCode } = useUserAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      setError('');
      await verifyNewEmailCode(newEmail, code, currentEmail);
  ...
  const handleResend = async () => {
    if (!newEmail) {
      Alert.alert('Error', 'New email is missing.');
      return;
    }
    try {
      setResendLoading(true);
      setError('');
      await resendCode(newEmail);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerText}>Verify New Email</Text>
      </View>
